...
use network::{SdpNetType, parse_addrtype, parse_nettype, parse_unicast_addr};
...
fn parse_origin(value: &str) -> Result<SdpLine, SdpParserError> {
    ...
    let unicast_addr = try!(parse_unicast_addr(ot[5]));
    let o = SdpOrigin {
...
fn parse_connection(value: &str) -> Result<SdpLine, SdpParserError> {
    ...
    println!("connection: {}, {}", c.nettype, c.addr);
...
fn parse_sdp_line(line: &str) -> Result<SdpLine, SdpParserError> {
    ...
    match name.to_lowercase().as_ref() {
        ...
        "z" => parse_zone(value),
        _ => {
            Err(SdpParserError::ParserLineError {
            ...
            Err(e) => {
                match e {
                    ...
                    SdpParserError::ParserLineError {
                        ...
                        errors.push(SdpParserError::ParserLineError {
                                        message: x,
                                        line: y,
                                    })
                    }
                    SdpParserError::ParserUnsupported {
                        message: x,
                        line: y,
                    } => {
                        println!("Warning unsupported value encountered: {}\n in line {}",
                                 x,
                                 y);
                        warnings.push(SdpParserError::ParserUnsupported {
                                          message: x,
                                          line: y,
                                      });
                    }
                    SdpParserError::ParserSequence {
                        message: x,
                        line: y,
                    } => {
                        errors.push(SdpParserError::ParserSequence {
                                        message: x,
                                        line: y,
                                    })
                    }
